...
public class DbActor
{
  public static async Task<int> InsertContactHistoryItem(bool isRcvd, DateTime timeSent, string email, string firstName, string subject, string body)
  {
    try
    {
      ...
      _ = await OutlookToDbWindow.CheckInsertEMailEHistAsync(db, email, firstName, "", subject, body, timeSent, "..from std broadcast send", isRcvd ? "R" : "S"); // db.EHists.Add(new EHist { EMailID = email, RecivedOrSent = isRcvd ? "R" : "S", EmailedAt = timeSent, LetterSubject = subject, LetterBody = body, Notes = "", AddedAt = GigaHunt.App.Now });
