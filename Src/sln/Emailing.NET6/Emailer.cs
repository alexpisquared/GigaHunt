  ...
  public static async Task<bool> Send(string from, string trgEmailAdrs, string msgSubject, string msgBody, string[]? attachedFilenames = null, string? signatureImage = null)
  {
    ...
    try
    {
      using (var mailMessage = new MailMessage(cFrom, trgEmailAdrs, msgSubject, msgBody))
      {
        ...
        catch (Exception ex) { ex.Log($"Error emailing to: {trgEmailAdrs}"); throw; }           //tu: add to BPR___.cfg: <system.net><mailSettings><smtp deliveryMethod="Network" from="test@foo.com"><!--userName="pigida@aei.ca" password=""--><!--port="25"--><network host="mail.aei.ca" defaultCredentials="true"/></smtp></mailSettings></system.net>
      }
